...
if expand_button:
    # If the button is clicked, display the contents
    st.header("Visualization of the Greeks")
    st.pyplot(fig1)
    st.pyplot(fig_bep)
    st.pyplot(fig2)
    st.pyplot(fig3)
...
for K in strike_prices:
    if pnl_visibility[K]:  # Only add the trace if the checkbox for it is checked
        ...
        pnl_trace = go.Scatter(
            ...
            hoverinfo='text',
            text=[
                f'Strike Price: {K}<br>Underlying Price: {Si}<br>Underlying Price at Option Purchase: {initial_price:.2f}<br>Option Price at Purchase: {option_price_at_purchase:.2f}<br>P&L: {pnl:.2f}'
