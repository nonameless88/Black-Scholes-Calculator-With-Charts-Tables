...
def calculate_pnl(initial_price, Ks, r, T, sigma, type="c"):
    ...
    for K in Ks:
        ...
        if type == "c":
            ...
            pnl[K] = [Si - K - option_price_at_purchase if Si - K > option_price_at_purchase else -option_price_at_purchase for Si in spot_prices]
        elif type == "p":
            # For a put option, the P&L is Strike Price - Current Asset Price - option_price_at_purchase
            pnl[K] = [K - Si - option_price_at_purchase if K - Si > option_price_at_purchase else -option_price_at_purchase for Si in spot_prices]
