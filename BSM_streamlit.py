...
for K in strike_prices:
    if pnl_visibility[K]:  # Only add the trace if the checkbox for it is checked
        pnl_trace = go.Scatter(
            ...
            hoverinfo='text',
            text=[f'Strike Price: {K}<br>Underlying Price: {Si}<br>Option Price at Purchase: {option_price_at_purchase:.2f}<br>P&L: {pnl:.2f}' for Si, pnl in zip(spot_prices, pnl_data[K])],
